...
namespace OpenRasta.Plugins.Caching.Pipeline
{
  ...
  {
    static readonly Regex _strongEtag = new Regex(string.Format(@"^(?<value>""[{0}]*"")$", AbnfRegex.ETAG_C));
    static readonly Regex _weakEtag = new Regex(string.Format(@"^W/(?<value>""[{0}]*"")$", AbnfRegex.ETAG_C));
