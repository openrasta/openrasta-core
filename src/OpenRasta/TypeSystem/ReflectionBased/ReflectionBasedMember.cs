...
namespace OpenRasta.TypeSystem.ReflectionBased
{
    ...
    where T : IMemberBuilder
  {
    ...
    public virtual bool IsEnumerable => TargetType.IsArray ||
                                        (TargetType.Implements(typeof(IEnumerable<>)) &&
                                         !TargetType.Implements(typeof(IDictionary<,>)));
    ...
    public virtual IProperty GetIndexer(string indexerParameter)
    {
      ...
      return indexer != null
        ? SurrogateProperty(new ReflectionBasedProperty(TypeSystem, this, indexer.Value.Key, indexer.Value.Value))
        : null;
    }
